    ...
    public partial class Form1 : Form
    {
        ...
        {   
            ...
            {   
                ...
                if (i < 9)
                    ...
                    cards[i] = (PictureBox)this.Controls["card0" + (i + 1).ToString()];
                //if the card index is other than 9, which is bigger or equal to 10
                else
                    //show the card as just index + 1 
                    cards[i] = (PictureBox)this.Controls["card" + (i + 1).ToString()];
        ...
        private void StartButton_Click(object sender, EventArgs e)
        {
            ...
            {   
                ...
                //very random indeed
                int x = rand.Next(TOTAL_CARDS), y = rand.Next(TOTAL_CARDS);
                int temp = cardId[x];
                cardId[x] = cardId[y];
                cardId[y] = temp;
            }
        ...
        async private void Clicked_On_Card(object sender, EventArgs e)
        {
            ...
            if (cardsPicked < 2)
            {
                ...
                if (cardsPicked == 1)//if you picked one card already
                    prevCard = name;//previous card is set to name 
                
                //assign index values to already picked cards
                int index = ((name[name.Length - 2] - '0') * 10) + (name[name.Length - 1] - '0') - 1,
                    index2 = ((prevCard[prevCard.Length - 2] - '0') * 10) + (prevCard[prevCard.Length - 1] - '0') - 1;
                
                //call getCardById function to get the image of the card
                cards[index].Image = getCardById(cardId[index]);

                if (cardsPicked == 2)//if this is the second card
                {
                    await Task.Delay(2000);//wait for 2 seconds delay
                    
                    //if two cards are the same
                    if(getCardById(cardId[index]).Tag == getCardById(cardId[index2]).Tag)
                        //make them disappear
                        cards[index].Visible = cards[index2].Visible = false;
                    else//if two cards are different
                        //show their back again
                        cards[index].Image = cards[index2].Image = Properties.Resources.red_back;
